...
BEST_MODEL     = os.getenv("BEST_MODEL", "best_model.pth")
OUTPUT_DIR     = os.getenv("OUTPUT_DIR", MODEL_DIR)

BATCH_SIZE     = int(os.getenv("BATCH_SIZE", "32"))
NUM_WORKERS    = int(os.getenv("NUM_WORKERS", "4"))
OPT_THRESHOLD  = float(os.getenv("OPTIMAL_THRESHOLD", "0.5"))
...
class CascasTestDataset(Dataset):
    ...
    def _load_pair(self, root, rel_path, rand_idx):
        base = os.path.join(
            root,
            rel_path.split("/")[0] + "_cascas",
            rel_path.split("/")[1].split(".")[0],
        )
        casca_csv = os.path.join(base, f"casca{rand_idx}.csv")
        press_csv = os.path.join(base, f"matrix_pressure{rand_idx}.csv")
...
def compute_distances(dataloader):
    ...
    with torch.no_grad():
        for m1, pr1, m2, pr2, y in tqdm(dataloader):
            m1, pr1, m2, pr2 = m1.to(device), pr1.to(device), m2.to(device), pr2.to(device)
...
test_distances, test_labels, _, _ = compute_distances(test_loader)
y_pred_test = (test_distances >= OPT_THRESHOLD).astype(int)
...
# df_test.to_csv(os.path.join(OUTPUT_DIR, f"predictions_{suffix}.csv"), index=False)
